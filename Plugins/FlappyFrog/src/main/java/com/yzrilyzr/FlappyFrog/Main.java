...
public class Main implements SurfaceHolder.Callback,Runnable,MediaPlayer.OnCompletionListener,API.WindowInterface,View.OnTouchListener
{
    ...
    float meter=0;
	float JUMP=20,GRAVITY=1,SPEED=7,GAP=500,PREPARE=1500,CLOUDSPEED=2f,CLOUDSCALE=3F;
    Bitmap[] bmps;
    long time=0;
	...
	public Main(Context ctx,Intent intent) throws Throwable
    {
        ...
        API.invoke(window,"setInterface",this);
		bmps=new Bitmap[8];
    ...
    public boolean onTouch(View v,MotionEvent e)
    {
        ...
        if(e.getAction()==MotionEvent.ACTION_DOWN)
        {
			...
			else if(NOW==4&&restartObj.contains(e.getX(),e.getY()))
			{
				NOW=1;
				resetMap(v);
			}
